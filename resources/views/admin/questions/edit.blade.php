...
@section('content')
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Edit Question</h1>
            <p class="mt-1 text-slate-500">Update question details and answer options.</p>
        </div>

        {{-- Form Card --}}
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            <form action="{{ route('admin.questions.update', $question) }}" method="POST" class="p-6 space-y-6"
                x-data="questionForm()">
                @csrf
                @method('PUT')
                    ...
                    <div class="flex gap-4">
                        <label class="flex-1 cursor-pointer">
                            <input type="radio" name="difficulty" value="easy" class="peer sr-only"
                                {{ old('difficulty', $question->difficulty) == 'easy' ? 'checked' : '' }}>
                            <div
                        ...
                        <label class="flex-1 cursor-pointer">
                            <input type="radio" name="difficulty" value="medium" class="peer sr-only"
                                {{ old('difficulty', $question->difficulty) == 'medium' ? 'checked' : '' }}>
                            <div
                        ...
                        <label class="flex-1 cursor-pointer">
                            <input type="radio" name="difficulty" value="hard" class="peer sr-only"
                                {{ old('difficulty', $question->difficulty) == 'hard' ? 'checked' : '' }}>
                            <div
                ...
                <div>
                    ...
                    <textarea name="explanation" id="explanation" rows="3"
                        class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all resize-none"
                        placeholder="Explain why this is the correct answer...">{{ old('explanation', $question->explanation) }}</textarea>
                    @error('explanation')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
