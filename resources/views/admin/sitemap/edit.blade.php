...
@section('page-title', 'Edit Sitemap URL')
    ...
    <div class="max-w-3xl mx-auto">
        <div class="bg-white rounded-2xl border border-slate-200 shadow-premium overflow-hidden">
            ...
            <div class="px-6 py-5 border-b border-slate-200">
                <h3 class="text-lg font-bold text-slate-900">Edit URL</h3>
                <p class="text-sm text-slate-500 mt-1">Update sitemap URL settings</p>
            </div>

            <!-- Form -->
            <form action="{{ route('admin.sitemap.update', $sitemap) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="url" id="url" value="{{ old('url', $sitemap->url) }}"
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $sitemap->name) }}"
                        class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
                    ...
                    <div>
                        ...
                        <select name="type" id="type" required
                            ...
                            @foreach ($typeOptions as $value => $label)
                                <option value="{{ $value }}"
                                    {{ old('type', $sitemap->type) == $value ? 'selected' : '' }}>{{ $label }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="grid grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="changefreq" id="changefreq" required
                            ...
                            @foreach ($changefreqOptions as $value => $label)
                                <option value="{{ $value }}"
                                    {{ old('changefreq', $sitemap->changefreq) == $value ? 'selected' : '' }}>
                                    {{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="lastmod" id="lastmod"
                            value="{{ old('lastmod', $sitemap->lastmod?->format('Y-m-d')) }}"
                            class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" id="sort_order"
                        value="{{ old('sort_order', $sitemap->sort_order) }}"
                        class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all">
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="3"
                        class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
                        placeholder="Internal notes about this URL...">{{ old('notes', $sitemap->notes) }}</textarea>
                </div>

                <!-- Is Active -->
                <div class="flex items-center gap-3">
                    <input type="checkbox" name="is_active" id="is_active" {{ $sitemap->is_active ? 'checked' : '' }}
