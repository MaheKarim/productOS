...
@section('page-title', 'Edit Gift Offer')
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-6">
            <div class="flex items-center gap-3 mb-1">
                ...
                <h1 class="text-xl font-semibold text-slate-900">Edit Gift Offer</h1>
            </div>
            <p class="text-slate-500 text-sm ml-8">Update the promotional offer details for
                <strong>{{ $gift->website_name }}</strong>.</p>
        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div class="p-6 space-y-5">
                            ...
                            <div>
                                ...
                                <input type="text" id="website_name" name="website_name"
                                    value="{{ old('website_name', $gift->website_name) }}" required
                                    placeholder="e.g. Amazon, Udemy, Coursera"
                            ...
                            <div>
                                ...
                                <textarea id="short_description" name="short_description" rows="3" required
                                    ...
                                    class="w-full px-3 py-2.5 bg-white border border-slate-200 text-slate-900 placeholder-slate-400 rounded-lg focus:border-slate-400 focus:ring-2 focus:ring-slate-200 transition-all text-sm resize-none">{{ old('short_description', $gift->short_description) }}</textarea>
                            ...
                            <div>
                                ...
                                <input type="url" id="link" name="link" value="{{ old('link', $gift->link) }}"
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                ...
                                <div>
                                    ...
                                    <input type="text" id="offer_percentage" name="offer_percentage"
                                        value="{{ old('offer_percentage', $gift->offer_percentage) }}" required
                                ...
                                <div>
                                    ...
                                    <input type="number" id="sort_order" name="sort_order"
                                        value="{{ old('sort_order', $gift->sort_order) }}" placeholder="0"
                                        class="w-full h-10 px-3 bg-white border border-slate-200 text-slate-900 placeholder-slate-400 rounded-lg focus:border-slate-400 focus:ring-2 focus:ring-slate-200 transition-all text-sm">
                            ...
                            <div>
                                ...
                                <div class="flex items-center gap-4">
                                    <div id="logo-preview"
                                        ...
                                        @if ($gift->logo)
                                            <img src="{{ asset('storage/' . $gift->logo) }}"
                                                class="w-full h-full object-cover" alt="{{ $gift->website_name }}">
                                        @else
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <p class="mt-1 text-xs text-slate-400">Upload a new logo to replace the current one.
                                            JPG, PNG, SVG, or WebP. Max 2MB.</p>
                                    </div>
                                </div>
                                @error('logo')
                                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                ...
                <div class="lg:col-span-1 space-y-6">
                        ...
                        <div class="p-6 space-y-5">
                            ...
                            <div class="bg-slate-50 rounded-lg p-4 space-y-2">
                                <div class="flex items-center justify-between text-xs">
                                    ...
                                    <span
                                        class="text-slate-700 font-medium">{{ $gift->created_at->format('M d, Y') }}</span>
                                </div>
                                <div class="flex items-center justify-between text-xs">
                                    <span class="text-slate-500">Last Updated</span>
                                    <span
                                        class="text-slate-700 font-medium">{{ $gift->updated_at->diffForHumans() }}</span>
                                </div>
                            </div>
                    ...
                    <div class="bg-red-50 rounded-xl border border-red-100 p-5">
                        <h4 class="text-xs font-medium text-red-600 uppercase tracking-wider mb-3">Danger Zone</h4>
                        <form action="{{ route('admin.gifts.destroy', $gift) }}" method="POST"
                            onsubmit="return confirm('Are you sure you want to permanently delete this gift offer?')">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                                class="w-full h-9 px-4 bg-red-600 hover:bg-red-700 text-white font-medium rounded-lg transition-colors cursor-pointer flex items-center justify-center gap-2 text-sm">
