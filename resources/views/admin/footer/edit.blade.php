...
@section('content')
    <div class="max-w-5xl mx-auto">
        <div class="mb-10">
            ...
            <h3 class="text-3xl font-black text-slate-900 tracking-tight">Edit Footer</h3>
            <p class="text-slate-500 mt-2">Update footer content, links, and social media for your portfolio.</p>
        </div>
                    ...
                    <div class="bg-white rounded-[2.5rem] p-10 border border-slate-200/60 shadow-sm">
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <input type="text" name="logo_text" value="{{ old('logo_text', $footer->logo_text) }}"
                            ...
                            <div>
                                ...
                                <textarea name="description" rows="3"
                                    ...
                                    placeholder="Brief description of your brand or services...">{{ old('description', $footer->description) }}</textarea>
                            </div>
                            ...
                            <div>
                                ...
                                @if ($footer->logo_image)
                                    <div class="mb-4">
                                        <img src="{{ $footer->logo_image_url }}" alt="Current logo"
                                            class="w-32 h-32 object-cover rounded-xl border border-slate-200">
                                        <p class="text-xs text-slate-500 mt-2">Current logo</p>
                                    </div>
                                @endif
                                <div
                                    ...
                                    <div class="text-center group-hover:scale-105 transition-transform">
                                        ...
                                        <p class="text-[10px] font-bold text-slate-500">Drop new logo image or Browse</p>
                                        <p class="text-[10px] text-slate-400 mt-1">Leave empty to keep current</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <input type="url" name="github_url"
                                    value="{{ old('github_url', $footer->github_url) }}"
                                    class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white transition-soft"
                                ...
                                @for ($i = 0; $i < 4; $i++)
                                    <div class="space-y-2">
                                        <input type="text" name="column1_links[{{ $i }}][text]"
                                            value="{{ old('column1_links.' . $i . '.text', $footer->column1_links[$i]['text'] ?? '') }}"
                                        ...
                                        <input type="text" name="column1_links[{{ $i }}][url]"
                                            value="{{ old('column1_links.' . $i . '.url', $footer->column1_links[$i]['url'] ?? '') }}"
                                            class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-soft text-sm"
                                            placeholder="URL">
                                    </div>
                                @endfor
                                ...
                                @for ($i = 0; $i < 4; $i++)
                                    <div class="space-y-2">
                                        <input type="text" name="column2_links[{{ $i }}][text]"
                                            value="{{ old('column2_links.' . $i . '.text', $footer->column2_links[$i]['text'] ?? '') }}"
                                            class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-soft text-sm"
                                            placeholder="Link text {{ $i + 1 }}">
                                        <input type="text" name="column2_links[{{ $i }}][url]"
                                            value="{{ old('column2_links.' . $i . '.url', $footer->column2_links[$i]['url'] ?? '') }}"
                                            class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-soft text-sm"
                                            placeholder="URL">
                                    </div>
                                @endfor
                                ...
                                @for ($i = 0; $i < 4; $i++)
                                    <div class="space-y-2">
                                        <input type="text" name="column3_links[{{ $i }}][text]"
                                            value="{{ old('column3_links.' . $i . '.text', $footer->column3_links[$i]['text'] ?? '') }}"
                                            class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-soft text-sm"
                                            placeholder="Link text {{ $i + 1 }}">
                                        <input type="text" name="column3_links[{{ $i }}][url]"
                                            value="{{ old('column3_links.' . $i . '.url', $footer->column3_links[$i]['url'] ?? '') }}"
                                            class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-soft text-sm"
                                            placeholder="URL">
                                    </div>
                                @endfor
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input type="text" name="privacy_policy_url"
                                        value="{{ old('privacy_policy_url', $footer->privacy_policy_url) }}"
                                        class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white transition-soft"
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <input type="text" name="meta_title"
                                    value="{{ old('meta_title', $footer->meta_title) }}"
                                    class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white transition-soft"
                            ...
                            <div>
                                ...
                                <textarea name="meta_description" rows="3"
                                    class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white transition-soft resize-none"
                                    placeholder="SEO description for footer section">{{ old('meta_description', $footer->meta_description) }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="space-y-6">
                            <div
                                ...
                                <label class="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" name="is_active" value="1"
                                        {{ old('is_active', $footer->is_active) ? 'checked' : '' }} class="sr-only peer">
                                    <div
                            ...
                            <div>
                                <label
                                    ...
                                    Order</label>
                                <input type="number" name="order" value="{{ old('order', $footer->order) }}"
                                    min="0"
